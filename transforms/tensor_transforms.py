...
class Resize(object):
    ...
    def _resize(self, im, size, interp= "BILINEAR"):
        ...
        if interp not in (
            ...
            raise ValueError("unknown interp filter")
        ...
        if interp == "NEAREST":
            ...
            interp = PIL.Image.BOX
        elif interp == "HAMMING":
            interp = PIL.Image.HAMMING
        ...
        for i in range(newsize[0]):
            if len(im.shape) == 3:
                s = np.squeeze(im[i,:,:])
                #ret[i,:,:] = imresize(s, size, interp) #IMRESIZE DEPRECATED!
                ret[i,:,:] = np.array(PIL.Image.fromarray(s).resize(size, interp))
            elif len(im.shape) == 4:
                for j in range(newsize[1]):
                    #s = np.squeeze(im[i,j,:,:])
                    #ret[i,j,:,:] = imresize(s, size, interp) #IMRESIZE DEPRECATED!
                    ret[i,j,:,:] = np.array(PIL.Image.fromarray(s).resize(size, interp))
        if is_nparray:
            return ret
        else:
...
class ConstantRescale(object):

    def __init__(self, scaling):
        """
        Rescales input numpy.ndarray or tensor by a scaling basis vector
           out = input.*scaling (element wise)
        Arguments
        ---------
        scaling : tuple of same size as a sample (1,L)
        ...
        if isinstance(scaling, np.ndarray):
            ...
            assert torch.is_tensor(scaling), "ERROR in  ConstanRescale: input paremeter 'scaling' should be an instance" \
    ...
    def __call__(self, array):
        ...
        Arguments
        ---------
        tensor : numpy.ndarry or torch.Tensor
            Tensor of size (C, L)

        Returns:
        --------
        Tensor: element wise scaled by the scaling vector
        """
        if isinstance(array, np.ndarray):
            array= torch.tensor(array)
        else:
            assert torch.is_tensor(array), "ERROR in  ConstanRescale: input paremeter 'array' should be an instance" \
                                           " of <np.ndarray> or <torch.tensor>"

        assert array.shape[-1] == self.scaling.shape[-1], "ERROR in ConstanRescale: input numpy.ndarray and scaling" \
                                                           " have incongruent shapes: {} - {}".format(array.shape,self.scaling.shape)
        return (array*self.scaling).detach().numpy()

class NormalizeElementWise(object):

    def __init__(self, means, stds):
        """
        Normalizes input ndarray of size (L,) or (B,L) to a normal distribution with mean 0 and std 1 according to:
        X = (X-means)/stds (element wise on X, dimensions can be broadcasted)

        Arguments
        ---------
        -means : ndarray of L means of each component of input ndarray i.e. (mean1, mean2, mean3,...,meanL)
        -stds : ndarray of L means of each component of input ndarray i.e. (std1, std2, std3,...,stdL)

        :TODO: Add more error handling for safe usage (on shapes of inputs...)
        """
        assert isinstance(means, np.ndarray) or torch.is_tensor(means), "ERROR in {}: 'means' input parameter should be" \
                                                                        " an instance of 'numpy.ndarray' or 'torch.tenso" \
                                                                        "r'".format(self.getName())

        assert isinstance(stds, np.ndarray) or torch.is_tensor(stds), "ERROR in {}: 'stds' input parameter should be" \
                                                                        " an instance of 'numpy.ndarray' or 'torch.tenso" \
                                                                        "r'".format(self.getName())
        self.means = means
        self.stds = stds
    ...
    def __call__(self, *inputs):
        ...
        Arguments
        ---------
        *inputs : numpy ndarrays of shape (B,L) or (L,)
            labels to be normalized to the given self.out_range.
        Returns
        -------
        arrays normalized accordingly

        NOTE THAT BROADCASTING IS USED SO NO NEED TO ITERATE FOR MORE INPUTS
        """
        if isinstance(inputs, np.ndarray):
            return np.divide(np.subtract(inputs,self.means),self.stds).squeeze()
    ...
    def getName(self):
        return "NormalizeElementWise()"


class RescaleToRangeElementWise(object):

    def __init__(self, in_range, out_range = (0,1)):
        """
        Rescales input numpy.ndarray or tensor of size (1,L) from a given input range (a,b) to a given output range (c,d) according to:
        f(x) = c + [(d-c)/(b-a)]*(x-a)
           out = input.*scaling (element wise)
        Arguments
        ---------
        -in_range : tuple of L ranges of each component of the tensor i.e. ((min1,max1),(min2,max2),...,(minL,maxL))
        -out_range : tuple, output range common to all dimensions, (default : (0,1))

        :TODO: Add error handling for safe usage
        """

        self.in_range = np.array(in_range)
    ...
    def __call__(self, *inputs):
        ...
        ---------
        *inputs : numpy ndarrays of shape (B,L) or (L,)
            labels to be rescaled to the given self.out_range.
        Returns
        -------
        arrays reshaped accordingly
        """
        if len(inputs) > 1:
            outputs = []
            for _input in inputs:
                #TODO: Maybe there is a faster way to do this and some error checking would be nice
                out = [self.out_range[0] + (self.out_range[-1]-self.out_range[0])/(in_range[-1]-in_range[0])*(x-in_range[0]) for in_range,x in zip(self.in_range, _input)]
                outputs.append(out)
            return np.array(outputs).squeeze()
        else:
            out = [self.out_range[0] + (self.out_range[-1]-self.out_range[0])/(in_range[-1]-in_range[0])*(x-in_range[0]) for in_range,x in zip(self.in_range, inputs)]
            return np.array(out).squeeze()

    def getName(self):
        return "RescaleToRangeElementWise()"
